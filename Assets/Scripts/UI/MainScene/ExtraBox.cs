...
{
    public class ExtraBox : MainBaseUI
    {
        ...
        private void CoinEvent()
        {
            if (GameDataManager.DecreaseCoinCount(GameTool.getOnePropNeedCoin))
            {
                ...
                EventManager.Instance.TriggerEvent(GameEvent.GetProp3, 1);
        ...
        {
            GameVideoContor.ShowVideoAd(VedioAdType.购买清除孔位, VedioAdType.购买清除孔位.ToString(), delegate (bool isComplete, int a)
            {
                if (isComplete)
                {
                    GameDataManager.AddItemCount(ItemType.DoubleBox, 1);
